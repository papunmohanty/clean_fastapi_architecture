...
from src.exceptions import TodoCreationError, TodoNotFound
import logging
...
def create_todo(current_user: TokenData, db: Session, todo: models.TodoCreate) -> Todo:
    try:
        ...
        logging.error(f"Failed to create todo for yser {current_user.get_uuid()}. Error {str(e)}")
...
def get_todos(current_user: TokenData, db: Session) -> list[models.TodoResponse]:
    ...
    logging.info(f"Retrived {len(todos)} todos for user: {current_user.user_id()}")
    return todos

def get_todo_by_id(current_user: TokenData, db: Session, todo_id: UUID) -> Todo:
    todo = db.query(Todo).filter(Todo.id == todo_id).filter(Todo.user_id == current_user.get_uuid())
    if not todo:
        logging.warning(f"Todo {todo_id} not found for user {current_user.get_uuid()}")
        raise TodoNotFoundError(todo_id)
    logging.info(f"Retrived todo {todo_id} for user: {current_user.get_uuid()}")
    return todo
